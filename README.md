...
An example of how the project should be structured is below. `raw_data/` and `test_data/` are not included in the repository as they are too large for GitHub or generated by the workflow. Instructions for data download are included further below. 
...
│
├── processed_data/                <-- Inputs for the figure generation .qmd notebooks
│
├── figures/                       <-- CREATED by 02_failure_profile_clustering
...
quarto render 04_qPCR_expression_plots
```
